...
# Helper functions for dealing with users!
def createUser(login_session):
    # Create the User object to pass into the database
    newUser = User(name = login_session['username'], email = login_session['email'], picture = login_session['picture'])
    # Make a DB session, pass the user in, and commit the update
    session = DBSession()
    session.add(newUser)
    session.commit()
    # The user id was generated by the database, so we find it out by querying
    # for the value that was created. Then, we close the session and return the
    # user id for further use in the server code
    user = session.query(User).filter_by(email = login_session['email']).one()
    session.close()
    return user.id


# Get an object with all user info, given the user's id
...
# Get a user's ID given his/her email address. This will be used to connect the
# user to the right entry in our database upon login (because our user id is
# internal, and the way we know which one is right is by which email address is
# associated with the Google login)
def getUserId(email):
    try:
        session = DBSession()
